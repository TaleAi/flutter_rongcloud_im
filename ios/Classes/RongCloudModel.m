...
+ (NSDictionary *)createContentModel:(RCMessageContent *)messageContent
{
    ...
    if ([messageContent isKindOfClass:[RCTextMessage class]]) {
        ...
        _content = @{
                     ...
                     @"user": [RongCloudModel transferNULLToUserInfo:textMsg.senderUserInfo]
                     };
    }else if ([messageContent isKindOfClass:[RCImageMessage class]])
    {
        ...
        _content = @{
                     ...
                     @"user": [RongCloudModel transferNULLToUserInfo:imgMsg.senderUserInfo]
                     };
...
+ (NSDictionary *)transferNULLToUserInfo:(RCUserInfo *)value
{
    ...
    return @{
             @"id": value.userId,
             @"name": value.name,
             @"avatar": value.portraitUri,
             @"extra":value.extra
          };
}

+ (NSNumber *)RCTransferConversationTypeToInt:(RCConversationType)type
{
    NSNumber * _default = 0;
    if (type == ConversationType_PRIVATE) {
        _default = [NSNumber numberWithInteger:1];
    }else if (type == ConversationType_GROUP)
    {
        _default = [NSNumber numberWithInteger:3];
    }else if (type == ConversationType_DISCUSSION)
    {
        _default = [NSNumber numberWithInteger:2];
    }else if (type == ConversationType_CHATROOM)
    {
        _default = [NSNumber numberWithInteger:4];
    }else if (type == ConversationType_CUSTOMERSERVICE)
    {
        _default = [NSNumber numberWithInteger:5];
    }else if (type == ConversationType_SYSTEM)
    {
        _default = [NSNumber numberWithInteger:6];
    }
    return _default;
}
...
//    }
    ...
    NSNumber* _conversation_type = [RongCloudModel RCTransferConversationTypeToInt:conversation.conversationType];
...
+ (NSDictionary *)RCGenerateMessageModel:(RCMessage *)message
{
    ...
    NSNumber *_msgDirection = nil;
    if (MessageDirection_RECEIVE == message.messageDirection) {
        _msgDirection = [NSNumber numberWithInt:2];
    }else if (MessageDirection_SEND == message.messageDirection)
    {
        _msgDirection = [NSNumber numberWithInt:1];
    }
    NSNumber* _conversation_type = [RongCloudModel RCTransferConversationTypeToInt:message.conversationType];
    NSDictionary *_ret = @{
                           @"conversationType"     :   _conversation_type,
